...
from utils.mechanics import handle_succession, apply_db_updates, enqueue_delayed_action
...
class AdminCog(commands.Cog):
    ...
    @app_commands.command(name="admin_avancar_dia", description="[ADMIN] Força o reinício do dia, restaurando as ações de todos os reinos para 5.")
    @app_commands.checks.has_permissions(administrator=True)
    async def admin_avancar_dia(self, interaction: discord.Interaction):
        with self.bot.SessionLocal() as db:
            active_kingdoms = db.query(Kingdom).filter_by(is_active=True).all()
            if not active_kingdoms:
                await interaction.response.send_message("Não há reinos ativos para avançar o tempo.", ephemeral=True)
                return
            ...
            await interaction.response.send_message("🌞 **O Sol nasce novamente.** O tempo foi avançado e as ações de todos os reinos ativos foram restauradas para 5.", ephemeral=False)
